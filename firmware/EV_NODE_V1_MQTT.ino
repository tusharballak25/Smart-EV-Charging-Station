// ================================================================
//  EV-NODE v1 | INA219 + HiveMQ Edition | ESP32-C3
...
const char* TOPIC_TELEMETRY = "ev_vehicle/v1/telemetry";
const char* TOPIC_COMMANDS  = "ev_vehicle/v1/commands";
...
void connectMQTT() {
  ...
  String clientId = "EV_V1_" + String((uint32_t)ESP.getEfuseMac(), HEX);
...
void printDashboard() {
  Serial.println();
  Serial.println(F("╔═════════════════════════════════════════════════════════════╗"));
  Serial.println(F("║      ⚡ EV-NODE v1  |  INA219 + HiveMQ  |  ESP32-C3        ║"));
...
void setup() {
  ...
  Serial.println(F("[v1] INA219 OK — 32V/2A calibration set"));
  ...
  Serial.printf("[v1] No-load baseline: %.3fV\n", noLoadVoltage);
  ...
  Serial.println(F("[v1] Ready — publishing every 2s\n"));
}
