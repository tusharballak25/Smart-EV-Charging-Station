// ================================================================
//  EV-NODE v2 | INA219 + HiveMQ Edition | ESP32-C3
...
const char* TOPIC_TELEMETRY = "ev_vehicle/v2/telemetry";
const char* TOPIC_COMMANDS  = "ev_vehicle/v2/commands";
...
void connectMQTT() {
  ...
  String clientId = "EV_v2_" + String((uint32_t)ESP.getEfuseMac(), HEX);
...
void printDashboard() {
  Serial.println();
  Serial.println(F("╔═════════════════════════════════════════════════════════════╗"));
  Serial.println(F("║      ⚡ EV-NODE v2  |  INA219 + HiveMQ  |  ESP32-C3        ║"));
...
void setup() {
  ...
  Serial.println(F("[v2] INA219 OK — 32V/2A calibration set"));
  ...
  Serial.printf("[v2] No-load baseline: %.3fV\n", noLoadVoltage);
  ...
  Serial.println(F("[v2] Ready — publishing every 2s\n"));
}
